...
def generate_new_docs_race(example_ds, questions_per_doc=2, save=False):
  ...
  for i in range(0, n_docs):
    new_docs = [*new_docs, *process_doc_race(articles[i], docs[i], questions_per_doc=questions_per_doc)]
    if save is not None:
        try:
...
if __name__ == '__main__':
    Dataset.from_pandas(load_dataset('race', 'middle', split='train').to_pandas().head(10000)).sort('example_id')
    generate_new_docs_race(example_ds, questions_per_doc=4, save=SAVE_PATH_RACE)
